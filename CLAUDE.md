...
src/play.js          game rules: stepping through a location (Back/Next/Finish), answers (text/number/choice), the starting challenge, one-location-at-a-time, the timed clues reveal, validation: pure
src/pack.js          seal/open the game content for the participant file: pure (scrambling, not encryption)
src/vault.js         password encryption (PBKDF2 + AES-GCM, Web Crypto) for the field tools and the starting challenge: pure
...
src/app.js           the game participants run: map (Google Map Tiles API or OSM fallback), GPS, HUD, starting challenge (password → challenges), location sheet (arrival → challenges, Back/Next, Finish location), clues & suspects screen, start screen; exposes hooks
...
src/admin.js         admin & dev tools: the draft, game text, starting challenge editor, locations (add/rename/reorder/delete) + challenges editor, clues/suspects/timing editor, export/import, emulator, recorder, replay (admin file only)
src/styles.css
...
- **The starting challenge is optional and opens at Begin, before any location.** `GAME.start` is `null` or `{ name, lockText, arrivalText, tasks, password }` (tasks shaped like a location's; `lockText` is the password screen's wording, empty for `Play.START_LOCK_TEXT`). Begin (not Continue) sets `progress.start = "locked"`; the password from the LoQuiz host sets `"open"`; Finish sets `"done"`. While locked or open, `Play.openable` gives the engine no locations and `Play.activate` refuses; the clues wait for it like a location in progress. It has no screen of its own: the password leads straight to its first challenge (`Play.startStage` always gives a challenge, never an arrival stage), with `arrivalText`, when the organiser wrote any, above that first one and no Back there. Its place is kept in `at[Play.START]`, and it doesn't count as a location. Passwords ignore case and extra spaces (`Play.normalizePassword`). Export replaces it with `{ name, lockText, sealed }` (those two are shown before the password, so they stay in the scrambled pack): the text and challenges encrypted with the password (`Vault`), so the file holds neither the password nor the challenges; the phone decrypts on unlock and keeps the password under `…:startpass` to reopen after a reload. Import asks for the password to bring it back. `progress.start` is absent for games without one, for teams that began before it was added, and once it's removed (teams waiting on it carry on). The admin file only opens it with **Try it here**. Its password box is allowed on a participant's screen, as is a challenge's own answer box (see answers above).
- **Hints, points and scoring stay out.** Nothing on a participant's screen may show points, a score, a submit button or a hint.
- **Answers are optional.** Most challenges are answered in LoQuiz; a challenge may also carry `answer`, and then the team must get it right here before Next (or Finish) will move them on. `{ kind:"text"|"number"|"choice" }` with `accept: [patterns]` (text: `*` stands for anything, so `*ple*` accepts anything containing "ple"; capitals and extra spaces ignored) or `options: [{id, text, correct}]`. Right answers live in `progress.solved[taskId]`; a wrong one changes nothing. `Play.markSolved` is how the admin's and field tools' testing shortcuts step past one. Never add points, hints or verdict wording: a browser test fails if a challenge **without** an answer shows an input, or if "points", "score", "submit" or "hint" appear on a participant's screen.
